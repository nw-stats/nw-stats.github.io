...
export function WarListCard({ war, goldStar }: WarListCardProp): JSX.Element {
    ...
    return (
        <Link to={`/wars/${war.id}`}>
            <div className="grid grid-cols-3 bg-surface-2 text-foreground rounded-lg">
                {/* Attacker */}
...
export function WarListCardSkeleton(): JSX.Element {
    return (
        <div className="grid grid-cols-3 bg-surface-1 text-foreground rounded-lg overflow-hidden animate-pulse">
            {/* Attacker */}
            <div className="flex items-center justify-center h-full text-center font-semibold text-lg relative">
                <div className="absolute inset-0 bg-surface3 w-1/12 rounded-l-lg" />
